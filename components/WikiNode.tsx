...
export const WikiNode = memo(({
  ...
  const ResizeHandles = () => {
    const handleBase = "absolute bg-white border border-blue-600 z-50 shadow-sm transition-transform hover:scale-125";
        ...
        <div className="absolute top-14 left-2 right-2 z-50 animate-in fade-in zoom-in-95 duration-200" onPointerDown={(e) => e.stopPropagation()}>
          <div className="glass-panel p-2 flex flex-col gap-2 shadow-2xl border border-blue-500/30">
            ...
            <div className="flex gap-1">
              <input
                ref={branchInputRef}
                className="flex-1 bg-white dark:bg-black/40 border border-zinc-200 dark:border-white/10 px-2 py-1.5 text-xs text-zinc-900 dark:text-zinc-200 outline-none focus:border-blue-500/50 placeholder:text-zinc-400 dark:placeholder:text-zinc-600"
                placeholder="Topic to expand on..."
                value={branchQuery}
                onChange={(e) => setBranchQuery(e.target.value)}
                onKeyDown={(e) => e.key === 'Enter' && submitBranch()}
              />
              <button onClick={submitBranch} className="bg-blue-600 hover:bg-blue-500 text-white p-1.5 transition-colors" title="Generate Branch">
                <ArrowRight size={14} />
              </button>
            </div>
          </div>
        ...
        <div className="absolute bottom-10 left-2 right-2 z-50 animate-in fade-in zoom-in-95 duration-200" onPointerDown={(e) => e.stopPropagation()}>
          <div className="glass-panel p-2 flex flex-col gap-2 shadow-2xl border border-blue-500/30">
            ...
            <div className="flex gap-1">
              ...
              />
              <button onClick={submitAiEdit} className="bg-blue-600 hover:bg-blue-500 text-white p-1.5 transition-colors" title="Apply AI Edit">
                <ArrowRight size={14} />
              </button>
            </div>
          </div>
