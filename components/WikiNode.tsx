...
export const WikiNode = memo(({
  ...
  // Font Family
  const effectiveHeaderFont = resolveFont(node.headerFontStyle, node, allNodes, globalHeaderFont, globalFont, 'header');
  const effectiveBodyFont = resolveFont(node.bodyFontStyle, node, allNodes, globalHeaderFont, globalFont, 'body');
  const effectiveCaptionFont = resolveFont(node.captionFontStyle, node, allNodes, globalHeaderFont, globalFont, 'caption');
  const effectiveGroupFont = resolveFont(node.bodyFontStyle || node.fontStyle, node, allNodes, globalHeaderFont, globalFont, 'body');

  // Colors
  const effectiveHeaderColor = resolveColor(node.headerColor, node, allNodes, globalHeaderColor || globalColor, 'header');
  const effectiveBodyColor = resolveColor(node.bodyColor, node, allNodes, globalColor, 'body');
  const effectiveCaptionColor = resolveColor(node.captionColor, node, allNodes, globalColor, 'caption');
  ...
  const effectiveCaptionSize = resolveFontSize(node.captionFontSize, node, allNodes, 11, 'caption');
