...
export const Toolbar: React.FC<ToolbarProps> = ({ 
  ...
  const btnClass = "text-zinc-500 dark:text-zinc-400 hover:bg-black/5 hover:text-zinc-900 dark:hover:bg-white/10 dark:hover:text-zinc-100 rounded-sm";
  const activeBtnClass = "bg-blue-500/20 text-blue-600 dark:text-blue-400 rounded-sm";
  ...
  // Enhanced glass menu style - removed rounded corners
  const glassMenuClass = "absolute bottom-full left-0 mb-2 w-48 glass-panel bg-white/90 dark:bg-black/90 shadow-2xl flex flex-col py-1 overflow-hidden animate-in fade-in zoom-in-95 duration-100 z-[60]";
    ...
    <div className="fixed bottom-8 left-8 z-50 flex flex-col gap-3 items-start pointer-events-none">
      ...
      <div className="pointer-events-auto flex items-end gap-2">
        ...
        <div className="flex items-center gap-2">
            ...
            <div className="relative glass-panel p-1 flex items-center gap-0.5">
                 <button onClick={() => setShowSaveMenu(!showSaveMenu)} className={`p-2 transition-colors flex items-center gap-1 ${showSaveMenu ? 'bg-black/5 text-zinc-900 dark:bg-white/10 dark:text-zinc-100' : btnClass}`} title="System, Save & Load Menu">
                      <Save size={18} />
                 </button>
                 
                 <button onClick={onOpenSettings} className={`p-2 transition-colors ${btnClass}`} title="Configure AI Provider & Settings">
                    <Settings size={18} />
                 </button>

                 {showSaveMenu && (
                     <div className={glassMenuClass}>
            ...
            <div className="flex items-center gap-0.5 glass-panel p-1">
                <button onClick={() => onAddNode('text', 'Untitled', '')} className={`p-2 transition-colors ${btnClass}`} title="Create a new Text Node">
                  <StickyNote size={18} />
                </button>
                
                <div className="relative">
                  <button onClick={() => fileInputRef.current?.click()} className={`p-2 transition-colors flex items-center gap-1 ${btnClass}`} title="Upload an Image Node">
                      <ImageIcon size={18} />
                  </button>
                </div>
        ...
        {!isCollapsed && (
            <div className="glass-panel px-1 py-1 flex items-center gap-0.5 animate-in slide-in-from-left-2 duration-300 fade-in">
              ...
              <button onClick={onToggleGrid} className={`p-2 transition-colors ${showGrid ? activeBtnClass : btnClass}`} title="Toggle Background Grid"><Grid size={18} /></button>
              <button onClick={onToggleSnap} className={`p-2 transition-colors ${snapToGrid ? activeBtnClass : btnClass}`} title="Toggle Snap to Grid"><Magnet size={18} /></button>
              
              <div className="w-px h-5 bg-zinc-200 dark:bg-white/10 mx-1"></div>
              <button onClick={onZoomOut} className={`w-8 h-8 flex items-center justify-center rounded-none transition-colors font-mono text-lg ${btnClass}`} title="Zoom Out">-</button>
              <button onClick={onZoomIn} className={`w-8 h-8 flex items-center justify-center rounded-none transition-colors font-mono text-lg ${btnClass}`} title="Zoom In">+</button>
              
              <div className="w-px h-5 bg-zinc-200 dark:bg-white/10 mx-1"></div>
               <button onClick={onToggleTheme} className={`p-2 transition-colors ${btnClass}`} title="Toggle Light/Dark Mode">
        ...
        <div className="glass-panel p-1">
           <button 
             ...
             className={`p-2 transition-colors hover:bg-black/5 dark:hover:bg-white/10 text-zinc-500 dark:text-zinc-400 rounded-sm`}
