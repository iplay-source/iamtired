...
export const SelectedNodeToolbar: React.FC<SelectedNodeToolbarProps> = ({
  ...
  const renderBackgroundColors = (
    ...
    // Determine if Gray Layer is effectively active
    // If key is set, use it. If not, default is active ONLY if no custom bg and no blur (standard node look)
    const hasCustomBg = current && current !== 'var(--node-bg-0)';
    const effectiveGrayLayer = grayLayerKey 
        ? (selectedNode[grayLayerKey] !== undefined 
            ? selectedNode[grayLayerKey] 
            : (!hasCustomBg && !isBlurry))
        : false;
  ...
  // Calculate default gray layer state for Text nodes
  const hasCustomHeaderBg = selectedNode.headerBackgroundColor;
  // Default is Active if no custom Header BG is set (so it overlays standard or body bg)
      ...
      <div className="flex items-center gap-1">
        {selectedNode.type === 'text' && (
          <div className="flex flex-col gap-1">
            ...
            <FormatToolbar 
              ...
              onRestoreDefaults={() => updateSelectedNodes({ backgroundColor: undefined, blur: defaultBlur, headerGrayLayer: undefined, headerBackgroundColor: undefined })}
            />
          </div>
        )}
        ...
        {selectedNode.type === 'group' && (
          <div className="flex flex-col gap-1">
             ...
             {/* 4. Background Color & Blur + Gray Layer for Header */}
             {renderBackgroundSection("GROUP BG", false, 'backgroundColor', 'blur', 'headerGrayLayer')}
          </div>
        )}
      </div>
    </div>
