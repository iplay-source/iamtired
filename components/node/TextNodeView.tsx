  ...
  return (
    <div className={`flex-1 overflow-hidden relative flex flex-col bg-white dark:bg-[#09090b] ${fontClass}`} onWheel={(e) => e.stopPropagation()}>
      {isEditing ? (
          ...
          <div 
              className="markdown-content flex-1 overflow-y-auto px-4 py-3 text-[15px] select-text cursor-default bg-white dark:bg-[#18181b] transition-colors pb-10"
              onDoubleClick={onDoubleClick}
              onPointerDown={(e) => e.stopPropagation()} 
          >
              {content ? (
      ...
      {!isEditing && (
        <div className="absolute bottom-0 left-0 right-0 h-9 border-t border-zinc-200 dark:border-white/10 flex items-center gap-1 px-2 bg-zinc-50/95 dark:bg-[#18181b]/95 backdrop-blur-md shrink-0 font-sans transition-all duration-300 transform translate-y-full opacity-0 group-hover:translate-y-0 group-hover:opacity-100 z-10">
          <button onClick={(e) => { e.stopPropagation(); onExpand(); }} className="flex items-center gap-1.5 text-[10px] font-semibold text-purple-600 dark:text-purple-400 hover:bg-purple-500/10 px-2 py-1 rounded-sm transition-colors uppercase tracking-wider"><Sparkles size={11} /> Expand with AI</button>
          <div className="w-px h-3 bg-zinc-300 dark:bg-white/10 mx-1"></div>
          <button onClick={(e) => { e.stopPropagation(); onEditAI(); }} className="flex items-center gap-1.5 text-[10px] font-semibold text-blue-600 dark:text-blue-400 hover:bg-blue-500/10 px-2 py-1 rounded-sm transition-colors uppercase tracking-wider"><Wand2 size={11} /> Edit with AI</button>
        </div>
      )}
    </div>
  );
