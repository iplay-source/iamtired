...
export const ImageNodeView: React.FC<ImageNodeViewProps> = ({
  ...
  onAskAI, onGenerateImage, aiPanel, isHovered, blur, backgroundColor
}) => {
  ...
  // VISUAL DESIGN POLICY: Default blur is FALSE.
  ...
  if (isBlurry) {
      if (backgroundColor) {
          captionStyle.backgroundColor = `color-mix(in srgb, ${backgroundColor}, transparent ${transparency})`;
      } else {
          // Rely on glass-panel for default bg to match HUD
      }
  } else {
      captionStyle.backgroundColor = backgroundColor || 'var(--node-bg-0)';
  }
